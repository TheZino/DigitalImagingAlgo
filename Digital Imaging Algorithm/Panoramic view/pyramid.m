function [ out ] = pyramid( im1, im2, mask1, mask2)
%  Pyramid Blending
    ...
    if nargin <= 2
        ...
        mask1 = imerode(mask1, se1);
        mask2 = imerode(mask2, se1);
        
        pirGm{1,1} = impyramid(mask1,'reduce');
    end
    ...
    if mask_ok
        ...
        pirB{1,1} = mask1.*pirL1{1,1} + mask2.*pirL2{1,1};
        pirB{1,2} = pirGm{1,1}.*pirL1{1,2} + (1-pirGm{1,1}).*pirL2{1,2};
        pirB{1,3} = pirGm{1,2}.*pirL1{1,3} + (1-pirGm{1,2}).*pirL2{1,3};
    end
    % Visualizzazione piramide
